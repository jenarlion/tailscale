...
// RateLimitedFn is a wrapper for RateLimitedFnWithTime that includes the
...
// rate limits.
func RateLimitedFnWithTime(logf Logf, f time.Duration, burst int, maxCache int, timeNow func() time.Time) Logf {
	if disableRateLimit {
		return logf
	}
	var (
